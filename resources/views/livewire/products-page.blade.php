                            ...
                            @foreach ($catgeories as $category)
                                <li class="mb-4" wire:key="{{$category->id}}">
                                    <label for="{{ $category->slug }}" class="flex items-center dark:text-gray-300">
                                        <input value="{{ $category->id }}" id="{{ $category->slug }}"  type="checkbox" class="w-4 h-4 mr-2" wire:model.live="selected_categories">
                            ...
                            @foreach ($brands as $brand)
                                <li class="mb-4" wire:key="{{$brand->id}}">
                                    <label for="{{ $brand->slug }}" class="flex items-center dark:text-gray-300"
                                        >
                                        <input value="{{ $brand->id }}" id="{{ $brand->slug }}" type="checkbox" class="w-4 h-4 mr-2" wire:model.live="selected_brands">
                        ...
                        <ul>
                            <li class="mb-4">
                                <label for="feature" class="flex items-center dark:text-gray-300">
                                    <input type="checkbox" id="feature" wire:model.live="featured" value="1" class="w-4 h-4 mr-2">
                        ...
                        <div>
                          <div class="font-semibold">{{Number::currency($price_range, 'PKR')}}</div>
                            <input type="range"
                                ...
                                max="500000" wire:model.live="price_range" value="500000" step="1000">
                                    ...
                                    <div class="relative bg-white">
                                        <a wire:navigate href="/products/{{ $product->slug }}" class="">
                                            <img src="{{ url('storage', $product->images[0]) }}"
                                                alt="{{ $product->name }}" class="object-contain w-full h-56 mx-auto ">
                                        </a>
                                    </div>
                                    <div class="p-3 ">
