                            ...
                            @foreach ($orders as $order)
                                @php
                                    ...
                                    if ($order->status == 'cancelled') {
                                        $status =
                                            '<span class="bg-red-700 py-1 px-3 rounded  text-white shadow">Cancelled</span>';
                                    }
                                    ...
                                    if ($order->payment_status == 'failed') {
                                        $paymentstatus =
                                            '<span class="bg-red-700 py-1 px-3 rounded  text-white shadow">Failed</span>';
                                    }
                                @endphp
                                <tr wire:key="{{ $order->id }}"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                        {{ Number::currency($order->grand_total, 'PKR') }}</td>
